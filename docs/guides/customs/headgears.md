...
!!! info "Headgear Sprite Files"
	Consist of `gender_name.act` and `gender_name.spr`, meaning 2 files per gender.
### Sprites  
...
!!! info "`accessoryid.lub`"
	...
	!!! note 
		Clients have a hardcoded View ID limit, depending on client, it might vary.  
...
change the `costume = false` to `costume = true` in the `itemInfo.lub`.