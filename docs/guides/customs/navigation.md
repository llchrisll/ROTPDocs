...
!!! info "`data\luafiles514\lua files\worldviewdata\`"
	Find the files `worldviewdata_table.lub` and `worldviewdata_language.lub` (if your files are inside a GRF, extract them).  
		
	1. Open your `worldviewdata_language.lub` and add the following line  
	...
	2. Now open your `worldviewdata_table.lub` and add the following line:  
...
!!! info "`data\luafiles514\lua files\datainfo\` (Optional)"
...
Open the `data\luafiles514\lua files\navigation` folder and add your custom map to the `navi_map_krpri.lub`. 

!!! info "`navi_map_krpri.lub`"
	Open the file, scroll all the way down the list and add your map in the following format:  
	`{ [mapname], Display name, ID, xSize, ySize }`

	!!! info
		If you don't have any files, you can use the `navigenerator.bat` included in the server folder which is generator if you build\compile it via your `rAthena.sln` and automatically generate them based on the NPC's and Maps which are available for you.
	...
	=== "xSize and ySize"
		Correspond to the values of gat.Width as well as gat.Height

	!!! info "Adding connections between neighbouring maps"
...
!!! info "Duplicate/Clone Maps"
	...
	Remember to either use a Map Cache Editor or the `mapcache.bat` from rAthena afterwards.